...
export default function Header() {
  ...
  return (
    <>
      ...
      {/* Notifications */}
      <div className="fixed top-20 right-2 sm:right-6 z-50 space-y-2 sm:space-y-3 pointer-events-none max-w-xs sm:max-w-sm px-2 sm:px-0">
        {notifications.map(notification => {
          ...
          return (
            <div
              key={notification.id}
              className={`${colors} border rounded-lg p-3 sm:p-4 flex items-center gap-3 shadow-lg pointer-events-auto animate-slideIn text-xs sm:text-sm`}
            >
              <Icon size={18} className="flex-shrink-0" />
              <span className="font-medium flex-1 min-w-0">{notification.message}</span>
              <button
                onClick={() => removeNotification(notification.id)}
                className="ml-2 text-lg opacity-50 hover:opacity-100 transition-opacity flex-shrink-0"
              >
                ×
              </button>
            </div>
